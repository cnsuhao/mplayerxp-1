...
static void resize(int x,int y)
{
    ...
    int w = x, h = y;
    aspect(&w,&h,A_ZOOM);
    dst_rect.left=( vo_screenwidth - (dwidth > vo_screenwidth?vo_screenwidth:dwidth) ) / 2;
    dst_rect.top=( vo_screenheight - (dheight > vo_screenheight?vo_screenheight:dheight) ) / 2;
...
{
    MSG_ERR("[vdpau] Display preemption detected\n");
    is_preempted = 1;
}
...
static int win_x11_init_vdpau_procs(void)
{
    ...
    if (vdp_st != VDP_STATUS_OK) {
        MSG_ERR("[vdpau] Error when calling vdp_device_create_x11: %i\n", vdp_st);
        return -1;
    }
    ...
    for (dsc = vdp_func; dsc->pointer; dsc++) {
        ...
        if (vdp_st != VDP_STATUS_OK) {
            MSG_ERR("[vdpau] Error when calling vdp_get_proc_address(function id %d): %s\n",
...
static int update_csc_matrix(void)
{
    ...
    MSG_V("[vdpau] Updating CSC matrix for %s\n",
           vdp_names[csp]);
                              ...
                              int max_refs)
{
    ...
    case IMGFMT_VDPAU_H264:
        ...
        MSG_V("[vdpau] Creating H264 hardware decoder for %d reference frames.\n", max_refs);
        break;
...
static int handle_preemption(void)
{
    if (!is_preempted)
        return 0;
    is_preempted = 0;
    MSG_INFO("[vdpau] Attempting to recover from preemption.\n");
    ...
    if (win_x11_init_vdpau_procs() < 0 ||
        ...
        MSG_ERR("[vdpau] Recovering from preemption failed\n");
        is_preempted = 1;
        return -1;
    }
//    resize();
    MSG_INFO("[vdpau] Recovered from display preemption.\n");
    return 1;
}
...
static uint32_t __FASTCALL__ config(uint32_t width, uint32_t height, uint32_t d_width,
                  ...
                  uint32_t format,const vo_tune_info_t *tune)
{
    XSizeHints hint;
    XVisualInfo vinfo;
...
static void draw_osd_I8A8(int x0,int y0, int w,int h, const unsigned char *src,
                          const unsigned char *srca, int stride, unsigned char* dstbase,int dststride)
{
...
//        return VO_FALSE;
    ...
    rndr = get_surface(0/*mpi->number*/);
    if (!rndr) {
        MSG_ERR("[vdpau] no surfaces available in get_image\n");
...
{
    int default_flags = VFCAP_CSP_SUPPORTED | VFCAP_CSP_SUPPORTED_BY_HW | VFCAP_HWSCALE_UP | VFCAP_HWSCALE_DOWN | VFCAP_OSD | VFCAP_FLIP;
    switch (format) {
    case IMGFMT_BGRA:
        if (force_mixer)
            return 0;
...
//        return;
    ...
    for (i = 0; i < MAX_VIDEO_SURFACES; i++) {
        ...
        free(surface_render[i].bitstream_buffers);
...
static uint32_t __FASTCALL__ preinit(const char *arg)
{
    ...
    if (colorspace < 0 || colorspace > 3) {
        MSG_WARN("[vdpau] Invalid color space specified, "
               "using BT.601\n");
        colorspace = 1;
    }

    if (!vo_init(arg) || win_x11_init_vdpau_procs())
        return -1;
