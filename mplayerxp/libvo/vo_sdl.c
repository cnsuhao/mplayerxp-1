...
static int __FASTCALL__ setup_surface(unsigned idx);
static int __FASTCALL__ set_video_mode(int width, int height, int bpp, uint32_t sdlflags);
static void __FASTCALL__ erase_rectangle(unsigned idx,int x, int y, int w, int h);
...
static int sdl_open ( void )
{
	...
	if(memcmp(sdl_subdevice,"vidix",5) == 0)
	{
		...
		if(vidix_preinit(vidix_name,&video_out_sdl)!=0) return -1;
...
static int __FASTCALL__ set_fullmode (int mode) {
	struct sdl_priv_s *priv = &sdl_priv;
...
//static int sdl_setup (int width, int height)
{
	...
	if(flags&ZOOM) {
		...
		&& priv->X)) {
            ...
            retval = set_video_mode(priv->dstwidth, priv->dstheight, priv->bpp, priv->sdlflags);
	    if(retval) return retval;
		}
		else {
			MSG_V("SDL: setting zoomed fullscreen with modeswitching\n");
...
static int setup_surfaces( void )
{
    ...
    {
	if(vidix_init(priv->width,priv->height,0,priv->y,
...
static void __FASTCALL__ sdl_dri_get_surface_caps(dri_surface_cap_t *caps)
{
    struct sdl_priv_s *priv = &sdl_priv;
...
static void __FASTCALL__ sdl_dri_get_surface(dri_surface_t *surf)
{
    struct sdl_priv_s *priv = &sdl_priv;
    if(priv->mode == YUV)
    {
