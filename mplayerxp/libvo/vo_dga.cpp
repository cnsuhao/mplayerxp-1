...
struct dga_priv_t : public video_private {
    dga_priv_t();
    virtual ~dga_priv_t() {}
...
inline struct vd_modes HW_MODE(dga_priv_t& priv) { return vo_dga_modes[priv.hw_mode]; }
inline struct vd_modes SRC_MODE(dga_priv_t& priv) { return vo_dga_modes[priv.src_mode]; }
...
{
    dga_priv_t& priv = *static_cast<dga_priv_t*>(vo->priv);
#ifdef HAVE_DGA2
...
static int __FASTCALL__ dga2_find_best_mode(vo_data_t*vo,unsigned wanted_width,unsigned wanted_height)
{
    ...
    priv.width = modelines[j].bytesPerScanline / HW_MODE(priv).vdm_bytespp ;
...
// choose a suitable mode ...
    ...
    MSG_V("vo_dga: Video parameters by codec: %3d x %3d, depth %2d, bitspp %2d.\n",
	    width, height,
	    SRC_MODE(priv).vdm_depth,
	    SRC_MODE(priv).vdm_bitspp);
/* now lets start the DGA thing */
  ...
  /* for both DGA1 and DGA2 we need to know ram_size */
    ...
    switch(HW_MODE(priv).vdm_bitspp) {
	...
	priv.vp_height, HW_MODE(priv).vdm_bytespp, priv.base,
	HW_MODE(priv).vdm_bitspp);

    priv.x_off = (priv.vp_width - priv.dest_width)>>1;
    priv.y_offset = priv.y_off = (priv.vp_height - priv.dest_height)>>1;
    priv.vp_offset = (priv.y_off * priv.width + priv.x_off ) * HW_MODE(priv).vdm_bytespp;
...
// set up variables for double buffering ...
    ...
    dest_frame_size = priv.width*HW_MODE(priv).vdm_bytespp*priv.vp_height;
...
static MPXP_Rc __FASTCALL__ preinit(vo_data_t*vo,const char *arg)
{
    ...
    dga_priv_t*priv=new(zeromem) dga_priv_t;
    vo->priv=priv;
    return MPXP_Ok;
}
...
static void __FASTCALL__ dga_dri_get_surface_caps(vo_data_t*vo,dri_surface_cap_t *caps)
{
    ...
    caps->strides[0] = priv.width*((HW_MODE(priv).vdm_bitspp+7)/8);
    caps->strides[1] = 0;
    caps->strides[2] = 0;
    caps->strides[3] = 0;
}
