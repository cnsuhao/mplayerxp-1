...
int mp_input_joystick_init(const char* dev) {
	...
	if(ev.type == JS_EVENT_BUTTON) btns |= (ev.value << ev.number);
	if(ev.type == JS_EVENT_AXIS)   axis[ev.number] = ev.value;
    }
    return fd;
}
...
int mp_input_joystick_init(const char* dev) { return -1; }
int mp_input_joystick_read(int fd) { return MP_INPUT_NOTHING; }
