...
//			printf("%d %d %d\n", dstWidth, i, srcW);
	...
	asm volatile(
		...
		"1:				\n\t"
		"mov %0, %%"REG_S"		\n\t"
		"movzb   (%%"REG_S", %%"REG_d"), %%"REG_D"	\n\t" //src[xx]
		"movzb  1(%%"REG_S", %%"REG_d"), %%"REG_S"	\n\t" //src[xx+1]
		"sub  %%"REG_D", %%"REG_S"		\n\t" //src[xx+1] - src[xx]
		"imul  %%"REG_c", %%"REG_S"		\n\t" //(src[xx+1] - src[xx])*2*xalpha
		"shl  $16, %%"REG_D"		\n\t"
		"add  %%"REG_D", %%"REG_S"		\n\t" //src[xx+1]*2*xalpha + src[xx]*(1-2*xalpha)
...
/*		printf("%d %d Last:%d %d LastInBuf:%d %d Index:%d %d Y:%d FSize: %d %d BSize: %d %d\n",
		...
		for(i=0; i<vLumFilterSize; i++)
		{
			...
			lumMmxFilter[4*i+1]= (int32_t)(((uint64_t)lumSrcPtr[i]) >> 32);
			lumMmxFilter[4*i+2]= 
			lumMmxFilter[4*i+3]= 
		...
		for(i=0; i<vChrFilterSize; i++)
		{
			...
			chrMmxFilter[4*i+1]= (int32_t)(((uint64_t)lumSrcPtr[i]) >> 32);
			chrMmxFilter[4*i+2]=
			chrMmxFilter[4*i+3]= 
