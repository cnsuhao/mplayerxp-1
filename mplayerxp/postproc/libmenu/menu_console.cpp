...
static m_option_t cfg_fields[] = {
  { "prompt", ST_OFF(mp_prompt), MCONF_TYPE_STRING, M_OPT_MIN, 1, 0, NULL },
  { "child-prompt", ST_OFF(child_prompt), MCONF_TYPE_STRING, M_OPT_MIN, 1, 0, NULL },
  { "buffer-lines", ST_OFF(buf_lines), MCONF_TYPE_INT, M_OPT_MIN, 5, 0, NULL },
  { "height", ST_OFF(height), MCONF_TYPE_INT, M_OPT_RANGE, 1, 100, NULL },
  { "minbor", ST_OFF(minb), MCONF_TYPE_INT, M_OPT_MIN, 0, 0, NULL },
  { "vspace", ST_OFF(vspace), MCONF_TYPE_INT, M_OPT_MIN, 0, 0, NULL },
  { "bg", ST_OFF(bg), MCONF_TYPE_INT, M_OPT_RANGE, -1, 255, NULL },
  { "bg-alpha", ST_OFF(bg_alpha), MCONF_TYPE_INT, M_OPT_RANGE, 0, 255, NULL },
  { "show-time",ST_OFF(show_time), MCONF_TYPE_INT, M_OPT_MIN, 0, 0, NULL },
  { "hide-time",ST_OFF(hide_time), MCONF_TYPE_INT, M_OPT_MIN, 0, 0, NULL },
  { "history-size",ST_OFF(history_max), MCONF_TYPE_INT, M_OPT_MIN, 1, 0, NULL },
  { "raw-child", ST_OFF(raw_child), MCONF_TYPE_FLAG, 0, 0, 1, NULL },
...
static void add_line(struct menu_priv_s* priv,const char* l) {
  ...
  if(eol) {
    char* _eol = mp_strdup(eol);
    if(_eol != l) {
      _eol[0] = '\0';
      add_line(priv,l);
    }
    if(_eol[1]) add_line(priv,_eol+1);
    delete _eol;
    return;
  }
...
static void add_string(struct menu_priv_s* priv, char* l) {
  ...
  priv->lines[ll] = (char*)mp_realloc(priv->lines[ll],strlen(priv->lines[ll]) + strlen(l) + 1);
...
static void enter_cmd(menu_t* menu) {
  ...
  if(mpriv->history == mpriv->cur_history) {
    ...
    h->size = 255;
    h->buffer = new(zeromem) char [h->size];
...
static void read_key(menu_t* menu,int c) {
  ...
  case KEY_ENTER: {
    ...
    cmd = mp_input_parse_cmd(mpriv->cur_history->buffer);
    enter_cmd(menu);
    if(!cmd)
...
#if 0
      case MP_CMD_RUN:
	run_shell_cmd(menu,cmd->args[0].v.s);
	break;
#endif
      default: // Send the other commands to mplayer
	mp_input_queue_cmd(menu->libinput,cmd);
      }
    }
    return;
  }
  ...
  if(isascii(c)) {
    ...
    if(l >= mpriv->cur_history->size) {
      ...
      mpriv->cur_history->buffer = (char*)mp_realloc(mpriv->cur_history,mpriv->cur_history->size);
    }
...
static int openMenu(menu_t* menu,const char* args) {
  ...
  mpriv->lines = new char* [mpriv->buf_lines];
  mpriv->prompt = mpriv->mp_prompt;
  mpriv->cur_history = mpriv->history = new(zeromem) history_t;
  mpriv->cur_history->buffer = new(zeromem) char [255];
  mpriv->cur_history->size = 255;
