...
void (* __FASTCALL__ adjust) (struct eq2_param_t *par, unsigned char *dst, unsigned char *src,
...
static void __FASTCALL__ affine_1d_MMX (eq2_param_t *par, unsigned char *dst, unsigned char *src,
  unsigned w, unsigned h, unsigned dstride, unsigned sstride)
{
  unsigned i;
...
static void __FASTCALL__ apply_lut (eq2_param_t *par, unsigned char *dst, unsigned char *src,
  unsigned w, unsigned h, unsigned dstride, unsigned sstride)
{
  unsigned      i, j, w2;
  unsigned char *lut;
...
static int __FASTCALL__ put_slice (vf_instance_t *vf, mp_image_t *src)
{
  ...
  dst = vf_get_new_image (vf->next, src->imgfmt, MP_IMGTYPE_EXPORT, 0,src->w, src->h,src->xp_idx);
...
static void __FASTCALL__ set_brightness (vf_eq2_t *eq2, double b)
{
  eq2->brightness = b;
  eq2->param[0].b = b;
  eq2->param[0].lut_clean = 0;
...
static void __FASTCALL__ set_gamma (vf_eq2_t *eq2, double g)
{
  eq2->gamma = g;
...
static void __FASTCALL__ set_saturation (vf_eq2_t *eq2, double s)
{
  eq2->saturation = s;
...
static int __FASTCALL__ query_format (vf_instance_t *vf, unsigned fmt,unsigned w,unsigned h)
{
  switch (fmt) {
    case IMGFMT_YVU9:
    case IMGFMT_IF09:
