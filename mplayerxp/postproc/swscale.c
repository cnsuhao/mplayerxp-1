...
static void *mpxp_malloc(unsigned int size)
{
    void *ptr;
#ifdef MEMALIGN_HACK
    long diff;
...
{
 mp_msg(0x10000, level, __FILE__, __LINE__, fmt );
}
...
char *sws_format_name(enum PixelFormat format)
{
    ...
    switch (format) {
        ...
        case PIX_FMT_NV21:
            return "nv21";
        default:
	    snprintf(ostr,sizeof(ostr),"Unknown format: %08X",format);
	    return ostr;
    }
}
			      ...
			      SwsVector *srcFilter, SwsVector *dstFilter, double param[2])
{
	...
	*filterPos = mpxp_malloc((dstW+1)*sizeof(int16_t));

	if(FFABS(xInc - 0x10000) <10) // unscaled
	{
		int i;
		filterSize= 1;
		filter= mpxp_malloc(dstW*sizeof(double)*filterSize);
...
/*				else if(flags & SWS_X)
	...
	*/
	...
	filter2= mpxp_malloc(filter2Size*dstW*sizeof(double));
	...
	ASSERT(filterSize > 0)
	filter= mpxp_malloc(filterSize*dstW*sizeof(double));
	...
	*outFilterSize= filterSize;
	...
	mpxp_free(filter2); filter2=NULL;
	...
	*outFilter= mpxp_malloc(*outFilterSize*(dstW+1)*sizeof(int16_t));
...
#if (defined(ARCH_X86) || defined(ARCH_X86_64)) && defined(CAN_COMPILE_MMX)
...
SwsContext *sws_getContext(int srcW, int srcH, int srcFormat, int dstW, int dstH, int dstFormat, int flags,
	...
	c= mpxp_malloc(sizeof(SwsContext));
...
// can't downscale !!!
...
#else
			c->funnyYCode = mpxp_malloc(MAX_FUNNY_CODE_SIZE);
			c->funnyUVCode = mpxp_malloc(MAX_FUNNY_CODE_SIZE);
#endif

			c->lumMmx2Filter   = mpxp_malloc((dstW        /8+8)*sizeof(int16_t));
			c->chrMmx2Filter   = mpxp_malloc((c->chrDstW  /4+8)*sizeof(int16_t));
			c->lumMmx2FilterPos= mpxp_malloc((dstW      /2/8+8)*sizeof(int32_t));
			c->chrMmx2FilterPos= mpxp_malloc((c->chrDstW/2/4+8)*sizeof(int32_t));
...
#ifdef HAVE_ALTIVEC
		c->vYCoeffsBank = mpxp_malloc(sizeof (vector signed short)*c->vLumFilterSize*c->dstH);
		c->vCCoeffsBank = mpxp_malloc(sizeof (vector signed short)*c->vChrFilterSize*c->chrDstH);
	...
	c->lumPixBuf= mpxp_malloc(c->vLumBufSize*2*sizeof(int16_t*));
	c->chrPixBuf= mpxp_malloc(c->vChrBufSize*2*sizeof(int16_t*));
	//Note we need at least one pixel more at the end because of the mmx code (just in case someone wanna replace the 4000/8000)
	/* align at 16 bytes for AltiVec */
	for(i=0; i<c->vLumBufSize; i++)
		c->lumPixBuf[i]= c->lumPixBuf[i+c->vLumBufSize]= mpxp_malloc(4000);
	for(i=0; i<c->vChrBufSize; i++)
		c->chrPixBuf[i]= c->chrPixBuf[i+c->vChrBufSize]= mpxp_malloc(8000);
...
SwsFilter *sws_getDefaultFilter(float lumaGBlur, float chromaGBlur, 
				...
				int verbose)
{
	SwsFilter *filter= mpxp_malloc(sizeof(SwsFilter));
...
SwsVector *sws_getGaussianVec(double variance, double quality){
	...
	int i;
	double *coeff= mpxp_malloc(length*sizeof(double));
	double middle= (length-1)*0.5;
	SwsVector *vec= mpxp_malloc(sizeof(SwsVector));
...
SwsVector *sws_cloneVec(SwsVector *a){
	double *coeff= mpxp_malloc(a->length*sizeof(double));
	int i;
	SwsVector *vec= mpxp_malloc(sizeof(SwsVector));
...
void sws_freeContext(SwsContext *c){
	...
	if(c->lumPixBuf)
	{
		for(i=0; i<c->vLumBufSize; i++)
		{
			mpxp_free(c->lumPixBuf[i]);
			c->lumPixBuf[i]=NULL;
		}
		mpxp_free(c->lumPixBuf);
		c->lumPixBuf=NULL;
	}
	...
	if(c->chrPixBuf)
	{
		for(i=0; i<c->vChrBufSize; i++)
		{
			mpxp_free(c->chrPixBuf[i]);
			c->chrPixBuf[i]=NULL;
		}
		mpxp_free(c->chrPixBuf);
		c->chrPixBuf=NULL;
	}

	mpxp_free(c->vLumFilter);
	c->vLumFilter = NULL;
	mpxp_free(c->vChrFilter);
	c->vChrFilter = NULL;
	mpxp_free(c->hLumFilter);
	c->hLumFilter = NULL;
	mpxp_free(c->hChrFilter);
	c->hChrFilter = NULL;
#ifdef HAVE_ALTIVEC
	mpxp_free(c->vYCoeffsBank);
	c->vYCoeffsBank = NULL;
	mpxp_free(c->vCCoeffsBank);
	c->vCCoeffsBank = NULL;
#endif

	mpxp_free(c->vLumFilterPos);
	c->vLumFilterPos = NULL;
	mpxp_free(c->vChrFilterPos);
	c->vChrFilterPos = NULL;
	mpxp_free(c->hLumFilterPos);
	c->hLumFilterPos = NULL;
	mpxp_free(c->hChrFilterPos);
...
#else
	mpxp_free(c->funnyYCode);
	mpxp_free(c->funnyUVCode);
#endif
	c->funnyYCode=NULL;
	c->funnyUVCode=NULL;
#endif /* defined(ARCH_X86) || defined(ARCH_X86_64) */

	mpxp_free(c->lumMmx2Filter);
	c->lumMmx2Filter=NULL;
	mpxp_free(c->chrMmx2Filter);
	c->chrMmx2Filter=NULL;
	mpxp_free(c->lumMmx2FilterPos);
	c->lumMmx2FilterPos=NULL;
	mpxp_free(c->chrMmx2FilterPos);
	c->chrMmx2FilterPos=NULL;
	mpxp_free(c->yuvTable);
	c->yuvTable=NULL;
