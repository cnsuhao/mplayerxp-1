...
struct nuv_private_t : public Opaque {
    nuv_private_t();
    virtual ~nuv_private_t();

    video_decoder_t* parent;
    sh_video_t* sh;
};
nuv_private_t::nuv_private_t() {}
nuv_private_t::~nuv_private_t() {}
...
static MPXP_Rc control_vd(Opaque& ctx,int cmd,any_t* arg,...){
    nuv_private_t& priv=static_cast<nuv_private_t&>(ctx);
...
static Opaque* preinit(const video_probe_t& probe,sh_video_t *sh,put_slice_info_t& psi){
    UNUSED(probe);
    UNUSED(psi);
    nuv_private_t* priv = new(zeromem) nuv_private_t;
    priv->sh=sh;
    return priv;
}
...
static mp_image_t* decode(Opaque& ctx,const enc_frame_t& frame){
    ...
    mpi=mpcodecs_get_image(*priv.parent, MP_IMGTYPE_TEMP, 0,
	sh->src_w, sh->src_h);
