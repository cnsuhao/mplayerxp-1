...
struct vorbis_private_t : public Opaque {
    vorbis_private_t();
    virtual ~vorbis_private_t();
...
vorbis_private_t::vorbis_private_t() {}
vorbis_private_t::~vorbis_private_t() {}
...
static Opaque* preinit(const audio_probe_t& probe,sh_audio_t *sh,audio_filter_info_t& afi)
{
    ...
    vorbis_private_t* priv = new(zeromem) vorbis_private_t;
    priv->sh = sh;
    priv->afi = &afi;
    return priv;
}
...
{
    vorbis_private_t& priv=static_cast<vorbis_private_t&>(ctx);
    ogg_packet op;
    vorbis_comment vc;
    float pts;
