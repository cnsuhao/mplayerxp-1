...
// dec_video.c:
extern any_t*	__FASTCALL__ mpcv_init(sh_video_t *sh_video, const char *codec_name,const char *family,int status,any_t*libinput);
extern void	__FASTCALL__ mpcv_uninit(any_t *handle);
extern any_t*	__FASTCALL__ mpcv_lavc_init(sh_video_t*,any_t* libinput);
extern int	__FASTCALL__ mpcv_decode(any_t *handle,const enc_frame_t* frame);
