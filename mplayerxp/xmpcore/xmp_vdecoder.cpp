...
if(ada_active_frame) /* don't emulate slow systems until xp_players are not started */
{
	...
	if(drop_param) mpxp_context().video().decoder->set_quality(0);
	else
	if(mp_conf.autoq) mpxp_context().video().decoder->set_quality(our_quality>0?our_quality:0);
    }
    frame->flags=drop_param;
    blit_frame=mpxp_context().video().decoder->run(*frame);
    mpxp_dbg2<<"DECODER: "<<dae_curr_vdecoded(mpxp_context().engine().xp_core)<<"["<<frame->len<<"] "<<frame->pts<<std::endl;
    if(mpxp_context().output_quality) {
	if(drop_param) mpxp_context().video().decoder->set_quality(mpxp_context().output_quality);
    }
