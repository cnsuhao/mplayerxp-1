...
static int ty_tmf_filetoparts( Demuxer *demux, TiVoInfo *tivo )
{
   ...
   while (!demux->stream->eof)
   {
	 ...
	 break;
      ...
      MSG_DBG3( "name %-20.20s size %-12.12s %d\n",
	 name, sizestr, size );
   ...
   // MPEG
	 ...
	 *ptsLen = SERIES1_PTS_LENGTH;
	 ...
	 if ( (size & 15) == (SERIES2_PTS_LENGTH & 15) )
	 {
      ...
      MSG_DBG3( "ty:Tossing Audio Packet Size %d\n",
	 size );
      return 0;
}
...
{
  int a = buf[0] & 0xe;
  int b = be2me_16(*((uint16_t*)(buf + 1)));
  int c = be2me_16(*((uint16_t*)(buf + 3)));
...
static int ty_demux( Demuxer *demux, Demuxer_Stream *dsds )
{
...
#endif
   {
	 ...
	 {
	    MSG_DBG3( "ty:Detected a tmf\n" );
	    tivo->tmf = 1;
	    ty_tmf_filetoparts( demux, tivo );
	    readSize = tmf_load_chunk( demux, tivo, chunk, 0 );
	 }

	 if ( readSize == CHUNKSIZE && be2me_32(*((uint32_t*)chunk)) == TIVO_PES_FILEID )
	 {
	       off_t numberParts;
   ...
   if (be2me_32(*((uint32_t*)chunk)) == TIVO_PES_FILEID)
      MSG_DBG3( "ty:Skipping PART Header\n" );
   ...
   // ======================================================================
   MSG_DBG3( "ty:ty parsing a chunk\n" );
   numberRecs = chunk[ 0 ];
   recPtr = &chunk[ 4 ];
	 ...
	 if ( size > 0 && size + offset <= CHUNKSIZE )
	 {
	    if( demux->audio->id == -1 )
	    {
		  ...
		  continue;    // DTiVo inconclusive, wait for more
	       ...
	       {
		  MSG_DBG3( "ty:Setting AC-3 Audio\n" );
		  aid = 0x80;  // AC-3
	       }
	       else
	       {
		  MSG_DBG3( "ty:Setting MPEG Audio\n" );
		  aid = 0x0;   // MPEG Audio
	       }
...
static Opaque* ty_open(Demuxer* demuxer)
{
    ...
    sh_video=reinterpret_cast<sh_video_t*>(demuxer->video->sh);
