...
int dvd_css_descramble ( u_char *sec , u_char *key )
{
...
static int path_to_lba (char *path)
{
    ...
    if (fp) {
	    ...
	    fgets(cmd,99,fp);
	    if ((ret=pclose(fp)))
		    MSG_V("DVD: fibmap_mplayer: %s\n",*cmd?cmd:"no error info");
	    if (cmd[0]<'0'||cmd[0]>'9') fp=NULL; else {
		if(WIFEXITED(ret) && !WEXITSTATUS(ret)) {
		    lba=atoi(cmd);
		    MSG_OK("DVD: fibmap_mplayer is being used\n");
		} else
		    fp=NULL;
	    }
    }
    if (!fp) {
	int fd;
	MSG_ERR("DVD: fibmap_mplayer could not run, trying with ioctl() ...\n");
	if ((fd = open(path, O_RDONLY)) == -1) {
    	    MSG_ERR( "DVD: Cannot open file %s: %s",
	    path ? path : "(NULL)", strerror(errno));
    	    return -1;
	}
        if (ioctl(fd, FIBMAP, &lba) != 0) {
            perror("DVD: ioctl FIBMAP");
	    MSG_HINT("  Hint: run mplayer as root (or better to install fibmap_mplayer as suid root)!\n");
            close(fd);
            return -1;
        }
	close(fd);
...
#warning translating pathname to iso9660 LBA is not supported on this platform
    MSG_ERR( "DVD: Translating pathname to iso9660 LBA is not supported on this platform\n");
    return -1;
}
...
int dvd_auth ( char *dev , char *filename )
{
	...
	if (!css_so) css_so=(char *)strdup("libcss"SLIBSUFFIX);
	MSG_V("DVD: opening libcss.so as %s ...\n",css_so);
	dlid=dlopen(css_so,RTLD_NOW);
	if (!dlid) {
		MSG_ERR("DVD: dlopen: %s\n",dlerror());
		return 1;
	} MSG_V("DVD: dlopen OK!\n");

#ifdef __OpenBSD__
#define CSS_DLSYM(v,s) if (!(v=ld_sym(dlid,"_" s))) {\
MSG_HINT("DVD: %s\n  Hint: use libcss version 0.1!\n",dlerror());\
return 1; }
#else
#define CSS_DLSYM(v,s) if (!(v=ld_sym(dlid,s))) {\
MSG_HINT("DVD: %s\n  Hint: use libcss version 0.1!\n",dlerror());\
return 1; }
#endif
...
#undef CSS_DLSYM
	...
	if ((dvd=DVDOpenDevice(dev)) == DVDOpenFailed) {
		MSG_ERR("DVD: cannot open DVD device \"%s\": %s.\n",
			dev, strerror(errno));
		return 1;
	}

	if (!(*dl_CSSisEncrypted)(dvd)) {
		MSG_V("DVD: DVD is unencrypted! Skipping authentication!\n(note: you should not use -dvd switch for unencrypted discs!)\n");
		DVDCloseDevice(dvd);
		return 0;
	} else MSG_V("DVD: DVD is encrypted, issuing authentication ...\n");
	/* reset AGIDs */
	reset_agids(dvd);
	/* authenticate disc */
	if ((*dl_CSSAuthDisc)(dvd,key_disc)) {
		MSG_ERR("DVD: CSSDVDAuthDisc() failed.\n");
		DVDCloseDevice(dvd);
		return 1;
	}
        if (CSSDVDAuthTitlePath(dvd,key_title,filename)) {
		MSG_ERR("DVD: CSSDVDAuthTitle() failed.\n");
		DVDCloseDevice(dvd);
		return 1;
	}
	...
	/* decrypting title */
        if ((*dl_CSSDecryptTitleKey)(key_title, key_disc) < 0) {
                MSG_ERR("DVD: CSSDecryptTitleKey() failed.\n");
		DVDCloseDevice(dvd);
		return 1;
	}

	DVDCloseDevice(dvd);
	MSG_V("DVD: DVD title key is: %02X%02X%02X%02X%02X\n",key_title[0],key_title[1],key_title[2],key_title[3],key_title[4]);
	descrambling=1;
	return 0;
}
