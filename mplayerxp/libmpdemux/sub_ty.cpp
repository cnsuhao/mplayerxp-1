...
static void ty_AddXDSToDisplay( const char *format, ... )
{
   ...
   {
      MSG_ERR( "XDS Buffer would have been blown\n" );
   }
   ...
   ty_XDS_Display[ ty_XDSAddLine ] = new char [strlen(line)+1];
...
static int ty_XDSdecode( char b1, char b2 )
{
   ...
   if ( b1 == 0x0F )
   {
	 ...
	 MSG_DBG3( "%% %s ", TY_XDS_modes[ TY_XDS_mode ] );
...
#define DST               ( ( TY_XDS[ 3 ][ 4 ][ 0 ] & 0x20 ) >> 5 )
	       struct tm tm;
		tm.tm_sec = 0;                                // sec
		tm.tm_min = ( TY_XDS_ptr[ 0 ] & 0x3F );       // min
		tm.tm_hour = ( TY_XDS_ptr[ 1 ] & 0x1F );      // hour
		tm.tm_mday = ( TY_XDS_ptr[ 2 ] & 0x1F );      // day
		tm.tm_mon = ( TY_XDS_ptr[ 3 ] & 0x1f ) - 1;   // month
		tm.tm_year = ( TY_XDS_ptr[ 5 ] & 0x3f ) + 90; // year
		tm.tm_wday = 0;                               // day of week
		tm.tm_yday = 0;                               // day of year
		tm.tm_isdst = 0;                              // DST

	       time_t tim = mktime( &tm );
	       char *timestr;

	       tim -= ( ( TIMEZONE - DST ) * 60 * 60 );
	       timestr = ctime( &tim );
...
void ty_processuserdata(const unsigned char* buf, int len )
{
	...
	if ( mp_conf.subcc_enabled )
	{
		if ( tyOSDInited == 0 )
		{
			...
			{
				ty_OSD1.text[ index ] = new char [TY_CC_MAX_X];
				ty_OSD2.text[ index ] = new char [TY_CC_MAX_X];
			}
			ty_ClearOSD( 0 );
