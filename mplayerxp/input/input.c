...
mp_input_get_cmd_from_keys(int n,int* keys, int paused) {
  ...
  if(cmd == NULL) {
    ...
    if(n > 1) {
      int s;
      for(s=1; s < n; s++)
	MSG_WARN("-%s",mp_input_get_key_name(keys[s]));
    }
    MSG_WARN("                         \n");
    return NULL;
  }
  ...
  if(!ret) {
	...
	MSG_ERR("-%s",mp_input_get_key_name(key_down[s]));
    }
    MSG_ERR(" : %s             \n",cmd);
  }
  return ret;
...
if(n>0){
  ...
  for(i = last_loop + 1 ; i != last_loop ; i++) {
    ...
    if(code == MP_INPUT_ERROR)
      MSG_ERR("Error on key input fd %d\n",key_fds[i].fd);
  ...
  while(1) {
	...
	if(buffer == iter) {
	  if(eof && (buffer-iter) == bs)
	    MSG_ERR("Unfinished binding %s\n",iter);
	  else
	    MSG_ERR("Buffer is too small for this key name : %s\n",iter);
	  mp_input_free_binds(binds);
	  return 0;
	}
	...
	for(i = 1; keys[i] != 0 ; i++)
	  MSG_ERR("-%s",mp_input_get_key_name(keys[i]));
	MSG_ERR("\n");
	keys[0] = 0;
	if(iter > buffer) {
...
mp_input_init(void) {
  ...
  if(! mp_input_parse_config(file)) {
    ...
    if(! mp_input_parse_config(file))
      MSG_WARN("Falling back on default (hardcoded) input config\n");
  }
