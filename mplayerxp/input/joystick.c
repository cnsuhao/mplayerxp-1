...
int mp_input_joystick_init(const char* dev) {
  ...
  while(! inited) {
    ...
    if((unsigned int)l < sizeof(struct js_event)) {
      if(l > 0)
	MSG_ERR("Joystick : we loose %d bytes of data\n",l);
      break;
    }
...
int mp_input_joystick_read(int fd) {
  ...
  while((unsigned int)l < sizeof(struct js_event)) {
	...
	return MP_INPUT_NOTHING;
      if( r < 0)
	MSG_ERR("Joystick error while reading joystick device : %s\n",strerror(errno));
      else
	MSG_ERR("Joystick error while reading joystick device : EOF\n");
