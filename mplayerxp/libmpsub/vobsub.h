...
extern any_t*  __FASTCALL__ vobsub_open(const std::string& subname, const char *const ifo, const int force, any_t** spu);
...
extern int __FASTCALL__ vobsub_parse_ifo(any_t* vob, const std::string& name, unsigned int *palette, unsigned int *width, unsigned int *height, int force, int sid, std::string& langid);
extern std::string __FASTCALL__ vobsub_get_id(any_t* vob, unsigned int index);
extern int __FASTCALL__ vobsub_set_from_lang(any_t*vobhandle,const std::string& lang);
