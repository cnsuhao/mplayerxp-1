...
static int dca_subsubframe (dca_state_t * state)
{
    ...
    for (k = 0; k < state->prim_channels; k++)
    {
        for (l = 0; l < state->vq_start_subband[k] ; l++)
        {
            ...
            default: /* Undefined */
                fprintf (stderr, "Unknown quantization index codebook %i\n",q_type);
                return -1;
            }
    ...
    {
	unsigned ffff;
	ffff = bitstream_get (state, 16);
        if (0xFFFF == ffff) /* 0xFFFF */
        {
#ifdef LOG_DEBUG
        ...
        {
            fprintf( stderr, "Didn't get subframe DSYNC. Has %04X instead\n",ffff);
        }
    }
...
{ 
    ...
    {
        fprintf (stderr, "ERROR: block code look-up failed %i\n",code);
        return 0;
    }
}
