...
/* It seems that 3dnow version of dct36 fixes some faults of non optimized stuff */
real   COS9[9];
real COS6_1,COS6_2;
real   tfcos36[9];
...
static int III_get_side_info(struct III_sideinfo *si,int stereo,
 int ms_stereo,uint32_t sfreq,int single,int lsf,int mpeg25)
{
   int ch, gr;
...
#define BITSHIFT ((sizeof(uint32_t)-1)*8)
#define REFRESH_MASK \
  while(num < (int)BITSHIFT) { \
    mask |= ((uint32_t)getbyte())<<(BITSHIFT-num); \
    num += 8; \
    part2remain -= 8; }
   ...
   struct gr_info_s *gr_info,int sfreq,int part2bits)
{
  ...
  mask  = ((uint32_t) getbits(num))<<BITSHIFT;
  mask <<= 8-num;
  part2remain -= num;

  {
	...
	fprintf(stderr,"You got some really nasty file there... region1(%i)>region2(%i)!\n",region1,region2);
	return 0;
    }
 ...
 */
  ...
  if(gr_info->block_type == 2) {
    ...
    for(;l3 && (part2remain+num > 0);l3--) {
      ...
      {
        fprintf(stderr,"attempted xrpnt overflow (%p !< %p)", (void*) xrpnt, (void*) &xr[SBLIMIT][0]);
        return 2;
      }
#endif
      if(!(xrpnt < &xr[SBLIMIT][0]+5))
   ...
   int ch,struct gr_info_s *gr_info)
{
   ...
   if(bt == 2) {
     for (; sb<gr_info->maxb; sb+=2,tspnt+=2,rawout1+=36,rawout2+=36) {
       fr.dct12(fsIn[sb],rawout1,rawout2,win[2],tspnt);
       fr.dct12(fsIn[sb+1],rawout1+18,rawout2+18,win1[2],tspnt+1);
     }
   }
   else {
     for (; sb<gr_info->maxb; sb+=2,tspnt+=2,rawout1+=36,rawout2+=36) {
...
int do_layer3(struct frame *fr,int single){
  int gr, ch, ss,clip=0;
