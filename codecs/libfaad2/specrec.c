...
** $Id: specrec.c,v 1.6 2007/12/14 08:51:08 nickols_k Exp $
...
#ifndef FIXED_POINT
ALIGN static const real_t __ALIGN pow2sf_tab[] = {
...
ALIGN static real_t __ALIGN pow2_table[] =
{
#if 0
...
static void apply_scalefactors_init(faacDecHandle hDecoder, ic_stream *ics,
                        real_t *x_invquant, uint16_t frame_len)
{
#ifdef CAN_COMPILE_X86_ASM
#ifdef CAN_COMPILE_SSE
	if(hDecoder->cpu_flags & MM_ACCEL_X86_SSE) apply_scalefactors = apply_scalefactors_SSE;
	else
#endif
#ifdef CAN_COMPILE_3DNOW
	if(hDecoder->cpu_flags & MM_ACCEL_X86_3DNOW) apply_scalefactors = apply_scalefactors_3DNow;
	else
#endif
#endif //CAN_COMPILE_X86_ASM
	apply_scalefactors = apply_scalefactors_c;
	(*apply_scalefactors)(hDecoder,ics,x_invquant,frame_len);
}

void (*apply_scalefactors)(faacDecHandle hDecoder, ic_stream *ics,
                        real_t *x_invquant, uint16_t frame_len)=apply_scalefactors_init;
...
static uint8_t allocate_channel_pair(faacDecHandle hDecoder,
                                     uint8_t channel, uint8_t paired_channel,element *cpe)
{
    uint8_t mul = 1;
                                 ...
                                 element *cpe, int16_t *spec_data1, int16_t *spec_data2)
{
    ...
    if (hDecoder->element_alloced[hDecoder->fr_ch_ele] == 0)
    {
        retval = allocate_channel_pair(hDecoder, cpe->channel, cpe->paired_channel,cpe);
        if (retval > 0)
            return retval;
