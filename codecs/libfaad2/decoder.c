...
** $Id: decoder.c,v 1.3 2004/10/27 18:13:10 nickols_k Exp $
...
unsigned faad_cpu_flags=0;

faacDecHandle FAADAPI faacDecOpen(unsigned cpu_flags)
{
    uint8_t i;
    ...
    faad_cpu_flags = hDecoder->cpu_flags = cpu_flags;
                            ...
                            uint32_t *samplerate, uint8_t *channels)
{
    ...
    *channels = 1;
    ...
    if (can_decode_ot(hDecoder->object_type) < 0)
        return -1;
    printf("Free AAC: Codec='%s' Optimiz=%s rate=%u ch=%u\n",
	    faac_subdecoders[hDecoder->object_type&0x1F],
	    hDecoder->cpu_flags & MM_ACCEL_X86_SSE ? "SSE" :
	    hDecoder->cpu_flags & MM_ACCEL_X86_3DNOW ? "3DNow!" : "generic",
	    *samplerate,
	    *channels);
    return bits;
}
                            ...
                            uint32_t *samplerate, uint8_t *channels)
{
    ...
    *samplerate = mp4ASC.samplingFrequency;
    ...
    printf("Free AAC2: Codec='%s' Optimiz=%s rate=%u ch=%u\n",
	    faac_subdecoders[hDecoder->object_type&0x1F],
	    hDecoder->cpu_flags & MM_ACCEL_X86_SSE ? "SSE" :
	    hDecoder->cpu_flags & MM_ACCEL_X86_3DNOW ? "3DNow!" : "generic",
	    *samplerate,
