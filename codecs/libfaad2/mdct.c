...
** $Id: mdct.c,v 1.6 2007/12/14 08:51:08 nickols_k Exp $
...
static void __faad_imdct_init(mdct_info *mdct, real_t *X_in, real_t *X_out)
{
#ifdef CAN_COMPILE_X86_ASM
#ifdef CAN_COMPILE_SSE
	if(faad_cpu_flags & MM_ACCEL_X86_SSE) faad_imdct = faad_imdct_SSE;
	else
#endif
#ifdef CAN_COMPILE_3DNOW
	if(faad_cpu_flags & MM_ACCEL_X86_3DNOW) faad_imdct = faad_imdct_3DNow;
	else
#endif
#endif //CAN_COMPILE_X86_ASM
	faad_imdct = faad_imdct_c;
	(*faad_imdct)(mdct,X_in,X_out);
}

void (*faad_imdct)(mdct_info *mdct, real_t *X_in, real_t *X_out)=__faad_imdct_init;
...
#ifdef LTP_DEC
static void __faad_mdct_init(mdct_info *mdct, real_t *X_in, real_t *X_out)
{
#ifdef CAN_COMPILE_X86_ASM
#ifdef CAN_COMPILE_SSE
	if(faad_cpu_flags & MM_ACCEL_X86_SSE) faad_imdct = faad_mdct_SSE;
	else
#endif
#ifdef CAN_COMPILE_3DNOW
	if(faad_cpu_flags & MM_ACCEL_X86_3DNOW) faad_imdct = faad_mdct_3DNow;
	else
#endif
#endif //CAN_COMPILE_X86_ASM
	faad_imdct = faad_mdct_c;
	(*faad_mdct)(mdct,X_in,X_out);
}
void (*faad_mdct)(mdct_info *mdct, real_t *X_in, real_t *X_out)=__faad_mdct_init;
#endif
