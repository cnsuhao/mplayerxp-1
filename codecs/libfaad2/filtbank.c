...
** $Id: filtbank.c,v 1.5 2007/12/14 08:51:08 nickols_k Exp $
...
void ifilter_bank_init(fb_info *fb, uint8_t window_sequence, uint8_t window_shape,
...
#ifdef CAN_COMPILE_X86_ASM
#ifdef CAN_COMPILE_SSE
	if(faad_cpu_flags & MM_ACCEL_X86_SSE) ifilter_bank = ifilter_bank_SSE;
	else
#endif
#ifdef CAN_COMPILE_3DNOW
	if(faad_cpu_flags & MM_ACCEL_X86_3DNOW) ifilter_bank = ifilter_bank_3DNow;
	else
#endif
#endif //CAN_COMPILE_X86_ASM
	ifilter_bank = ifilter_bank_c;
	(*ifilter_bank)(fb,window_sequence,window_shape,window_shape_prev,freq_in,time_out,overlap,object_type,frame_len);
}
void (*ifilter_bank)(fb_info *fb, uint8_t window_sequence, uint8_t window_shape,
                  uint8_t window_shape_prev, real_t *freq_in,
                  real_t *time_out, real_t *overlap,
                  uint8_t object_type, uint16_t frame_len)=ifilter_bank_init;
