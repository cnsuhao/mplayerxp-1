...
#define bit_ptr (decoder->bitstream_ptr)
    ...
    decoder->mpq_scale = decoder->mpq_scales[quantizer_scale_code];
...
do {									\
    if(decoder->mpq_store)                                              \
        decoder->mpq_store[decoder->mpq_stride*(decoder->v_offset>>4)   \
                    +(decoder->offset>>4)] = decoder->mpq_scale;        \
    decoder->offset += 16;						\
